...
function createRobotjsExecutor(): Executor {
  ...
  return {
    ...
    typeText(text: string) {
      ...
      // biome-ignore lint/suspicious/noControlCharactersInRegex: needed for ASCII check
...
function createCliclickExecutor(): Executor {
  ...
  return {
    ...
    getScreenSize() {
      const out = execSync(
        `osascript -l JavaScript -e 'ObjC.import("AppKit"); var s = $.NSScreen.mainScreen; var f = s.frame; JSON.stringify({w: f.size.width, h: f.size.height})'`,
      )
        .toString()
        .trim();
